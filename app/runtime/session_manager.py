...
class SessionManager:
    ...
    def launch(
        ...
        *,
        ...
        try:
            ...
            if self.profile_manager is not None and profile is not None:
                session.profile_lock = self.profile_manager.acquire_runtime_lock(
                    profile,
                    session_id=session_id,
                )
            launch_kwargs = {}
            ...
            if self.readiness_probe(launch_result):
                ...
                try:
                    self.launcher.stop(launch_result)
                finally:
                    self._release_profile_resources(session)
    ...
    def _release_profile_resources(self, session: SessionState) -> None:
        if session.profile_lock is not None:
            session.profile_lock.release()
            session.profile_lock = None
